...
class Maze
{
    const DEFAULT_MAZE_HEIGHT = 5;
    const DEFAULT_MAZE_WIDTH = 5;
    const WALL = '&#x2591;';
    const DOUBLE_WALL = '&#x2591;';//&#x2591;';
    const FLOOR = ' ';
    const DOUBLE_FLOOR = ' ';// ';
            ...
            if ($i % 2 == 0) {
                ...
                for ($j = 0; $j < $this->x; $j++) {
                    ...
                    if ($this->walls[$currWall++] == 1) {
                        echo self::DOUBLE_WALL;
                    } else {
                        echo self::DOUBLE_FLOOR;
                    }
                }
    ...
    private function renderToImage(): void
    {
        ...
        $image = imagecreatetruecolor($printRows + 100, $this->x + 2+100);
        $wallColor = imagecolorallocate($image, 255, 0, 0);
        $wallColor1 = imagecolorallocate($image, 0, 0, 255);
        $wallColor2 = imagecolorallocate($image, 0, 255, 0);
        $wallColor3 = imagecolorallocate($image, 255, 255, 0);
        $wallColor4 = imagecolorallocate($image, 0, 255, 255);
        $wallColor5 = imagecolorallocate($image, 127, 127, 127);
        $emptyColor = imagecolorallocate($image, 0, 0, 0);
        $emptyColor1 = imagecolorallocate($image, 255, 255, 255);
...
//                        echo self::WALL;
//                        echo self::FLOOR;
                        imagesetpixel($image, $i, $j, $wallColor);
                    } else {
//                        echo self::FLOOR;
                        imagesetpixel($image, $i, $j, $emptyColor);
                    }
                    if ($this->walls[$currWall] == 1) {
//                        echo self::DOUBLE_WALL;
//                        echo self::DOUBLE_FLOOR;
                        imagesetpixel($image, $i, $j, $wallColor1);
                        imagesetpixel($image, $i, $j+1, $wallColor1);
                        $currWall++;
                    } else {
//                        echo self::DOUBLE_FLOOR;
                        imagesetpixel($image, $i, $j, $emptyColor);
                        imagesetpixel($image, $i+1, $j, $emptyColor1);
                    }
                    imagesetpixel($image, $i, $j+1, $wallColor5);
                }
                // Bottom-right corner does not need bottom-right divider
                if ($i != ($printRows - 1)) {
//                    echo self::WALL;
                    imagesetpixel($image, ($printRows - 1), $i, $wallColor3);
                }
            } else {
                // Printing the cell itself
                for ($j = 0; $j < $this->x; $j++) {
...
//                    echo self::DOUBLE_FLOOR;
                    imagesetpixel($image,  $i, $j, $emptyColor);
                    imagesetpixel($image, $currWall, $i, $emptyColor);
                }
...
//                    echo self::WALL;
                    imagesetpixel($image, $currWall, $i, $wallColor);
                    $currWall++;
                }
            }
//            echo "<br />";
...
//        echo '<br />';
//        for ($y = 0;  $y < $this->y; $y++) {
//            for ($x = 0; $x < $this->x * 2 + 1; $x++) {
////                echo \sprintf(
////                    '%d,%d = %s',
////                    $x,
////                    $y,
////                    $this->walls[$x + $this->y * $y]
////                );
//                $wall = $this->walls[$x + $this->y * $y];
//                if ($wall == 1) {
//                    echo self::WALL;
//                } else {
//                    echo self::FLOOR;
